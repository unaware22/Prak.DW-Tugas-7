...
function tampilkanDaftarBuku() {
    ...
    daftarBuku.forEach((buku, index) => {
        ...
        divBuku.innerHTML = `
            <p class="font-normal text-lg mb-2 mt-2">${buku.tampilkanInfo()}</p>
        `;

        const btnTambahFavorit = document.createElement('button');
        btnTambahFavorit.textContent = "Tambah ke Favorit";
        btnTambahFavorit.className = "font-custom font-semibold bg-[#febbd4] text-[#c44171] border-2 border-black py-2 px-4 text-base shadow-custom inline-block cursor-pointer rounded-md hover:bg-[#818586] hover:shadow-custom-hover transform hover:translate-x-0.5 hover:translate-y-0.5 mt-2 mb-5";
        btnTambahFavorit.addEventListener('click', function() {
            tambahKeFavorit(index);
        });

        divBuku.appendChild(btnTambahFavorit);
...
function tampilkanBukuFavorit() {
    ...
    bukuFavorit.forEach((buku, index) => {
        ...
        divBuku.classList.add('buku');
        divBuku.innerHTML = `
            <p class="font-normal text-lg mt-2">${buku.tampilkanInfo()}</p>
        `;

        const btnHapusFavorit = document.createElement('button');
        btnHapusFavorit.textContent = "Hapus";
        btnHapusFavorit.className = "font-custom font-semibold bg-[#febbd4] text-[#c44171] border-2 border-black py-2 px-4 text-base shadow-custom inline-block cursor-pointer rounded-md hover:bg-[#818586] hover:shadow-custom-hover transform hover:translate-x-0.5 hover:translate-y-0.5 mt-5";
        btnHapusFavorit.addEventListener('click', function() {
            hapusDariFavorit(index);
        });

        divBuku.appendChild(btnHapusFavorit);
